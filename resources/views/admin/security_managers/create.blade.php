...
<div class="max-w-5xl mx-auto py-6 px-4">
    <div class="border-4 border-blue-900 rounded-2xl p-6 bg-blue shadow-[0_4px_12px_rgba(0,0,0,0.7)]">
        <h1 class="text-2xl font-bold text-blue-800 mb-4">+ Add Security Manager</h1>
        ...
        <form action="{{ route('admin.security_managers.store') }}" method="POST" class="bg-white p-6 shadow rounded-lg border border-blue-200">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="approver_dgm_id" class="w-full mt-1 border rounded px-3 py-2 @error('approver_dgm_id') border-red-500 @enderror" required>
                    <option value="">-- Select Approver --</option>
                    @foreach($dgms as $dgm)
                        <option value="{{ $dgm->id }}" {{ old('approver_dgm_id') == $dgm->id ? 'selected' : '' }}>{{ $dgm->name }}</option>
                    @endforeach
                </select>
                @error('approver_dgm_id')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
